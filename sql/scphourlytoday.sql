...
,SUM(CASE WHEN DIAMETER_RESULT_CODES IN ('5030','5031','4010','4012') THEN 1 ELSE 0 END) AS bsfail
,SUM(CASE WHEN DIAMETER_RESULT_CODES NOT IN ('2001','5030','5031','4010','4012') OR DIAMETER_RESULT_CODES IS NULL  THEN 1 ELSE 0 END) AS FAIL    
FROM SCPCDR.INTERNAL_CDR_07@scpcdr_prod_pk_public
WHERE M_MONTH ='{mon}' 
GROUP BY GROUP BY to_char(CDR_TIMESTAMP,'dd-mm-yyyy') ),
base_d1 AS (SELECT to_char(CDR_TIMESTAMP,'dd-mm-yyyy') AS cdr_date
,COUNT(TRANSACTION_ID) AS attempt
,SUM(CASE WHEN DIAMETER_RESULT_CODES IN ('5030','5031','4010','4012') THEN 1 ELSE 0 END) AS bsfail
,SUM(CASE WHEN DIAMETER_RESULT_CODES NOT IN ('2001','5030','5031','4010','4012') OR DIAMETER_RESULT_CODES IS NULL  THEN 1 ELSE 0 END) AS FAIL    
FROM SCPCDR.INTERNAL_CDR_{day}
WHERE M_MONTH ='{mon}' AND to_char(CDR_TIMESTAMP,'HH24') <= to_char(SYSDATE,'HH24')
GROUP BY to_char(CDR_TIMESTAMP,'dd-mm-yyyy')
UNION
SELECT to_char(CDR_TIMESTAMP,'dd-mm-yyyy') AS cdr_date
,COUNT(TRANSACTION_ID) AS attempt
...
SELECT to_char(CDR_TIMESTAMP,'dd-mm-yyyy') AS cdr_date,to_char(CDR_TIMESTAMP,'HH24') AS cdr_hour,DECODE(IS_ROAMING,1,'ROAMING',0,'NOT ROAMING') AS ROAMING_STATUS
 ...
 ,SUM(CASE WHEN DIAMETER_RESULT_CODES in ('5030','5031','4010','4012') THEN 1 ELSE 0 END) AS BSF
 ,SUM(CASE WHEN DIAMETER_RESULT_CODES NOT IN ('2001','5030','5031','4010','4012') OR DIAMETER_RESULT_CODES IS NULL  THEN 1 ELSE 0 END) AS FAIL
FROM SCPCDR.INTERNAL_CDR_08@scpcdr_prod_pk_public
WHERE M_MONTH ='{mon}' 
GROUP BY to_char(CDR_TIMESTAMP,'dd-mm-yyyy'),to_char(CDR_TIMESTAMP,'HH24'),DECODE(IS_ROAMING,1,'ROAMING',0,'NOT ROAMING'))
