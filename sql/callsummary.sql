...
UNION
SELECT DECODE(IS_ROAMING,1,'ROAMING',0,'NOT ROAMING') AS ROAMING_STATUS,SERVICE_KEY
 ,COUNT(TRANSACTION_ID) AS CALL_ATTEMPT
 ,SUM(CASE WHEN DIAMETER_RESULT_CODES='2001' THEN 1 ELSE 0 END) AS SUCC
 ,SUM(CASE WHEN DIAMETER_RESULT_CODES='5030' THEN 1 ELSE 0 END) AS BLCK
 ,SUM(CASE WHEN DIAMETER_RESULT_CODES='5031' THEN 1 ELSE 0 END) AS UNKS
...
WHERE M_MONTH ='{mon}' 
GROUP BY DECODE(IS_ROAMING,1,'ROAMING',0,'NOT ROAMING'),SERVICE_KEY)
SELECT TO_CHAR(sysdate,'dd-mm-yyyy') AS cdrdate,TO_CHAR(sysdate,'HH24') AS cdrhour,ROAMING_STATUS,SERVICE_KEY
   ,SUM(CALL_ATTEMPT) AS CALL_ATTEMP
   ,SUM(SUCC) AS SUCC
   ,SUM(BLCK) AS BLCK
   ,SUM(UNKS) AS UNKS
   ,SUM(SUSP) AS SUSP
   ,SUM(INSF) AS INSF
   ,SUM(FAIL) AS FAIL
   ,ROUND((SUM(SUCC)/SUM(CALL_ATTEMPT))*100,2) AS SUCCESS_RATE
   ,ROUND((SUM(BLCK)/SUM(CALL_ATTEMPT))*100,2) AS BLOCK_RATE
   ,ROUND((SUM(UNKS)/SUM(CALL_ATTEMPT))*100,2) AS UNKS_RATE
   ,ROUND((SUM(SUSP)/SUM(CALL_ATTEMPT))*100,2) AS SUSPEND_RATE
   ,ROUND((SUM(INSF)/SUM(CALL_ATTEMPT))*100,2) AS INSF_RATE
   ,ROUND((SUM(FAIL)/SUM(CALL_ATTEMPT))*100,2) AS FAILED_RATE
FROM base_statistic
GROUP BY TO_CHAR(sysdate,'dd-mm-yyyy'),TO_CHAR(sysdate,'HH24'),ROAMING_STATUS,SERVICE_KEY
